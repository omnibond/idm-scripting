...
function idm_query
{
  ...
  if ($association -ne $null) {
    ...
    $domaindn = "LDAP://"
    # append domain controller if available
    if ($dc -ne $null) {
       $domaindn += $dc + "/"
    }
    $tokens = $domain.split(".")
    ...
    # append DEST_DN
    $base += (idm_geteventvalue "DEST_DN")

    $names = idm_geteventvaluenames
    $filter = "(&(objectClass=$classname)"
    foreach ($name in $names) {
      if ($name -eq $null) { break }
	  if ($name -notmatch "^SEARCH_ATTRS" -and $name -match "^SEARCH_ATTR") {
  ...
  if ($res -ne $null) {
    $path = $res.Path.Substring(7)
    if ($path.IndexOf("/") -ne -1) {
      $path = $path.Substring($path.IndexOf("/") + 1)
    }
    if ($classname -eq "User") {
      # For users, only return instance if user has a mailbox
      # (Omit LDAP:// from path)
      if ($dc -ne $null) {
        $identity = Get-Mailbox $path -DomainController $dc -ErrorAction SilentlyContinue
      }
      else {
        $identity = Get-Mailbox $path -ErrorAction SilentlyContinue
      }
    }
    elseif ($classname -eq "Group") {
      # For groups, only return instance if group is a distribution group
      if ($dc -ne $null) {
        $identity = Get-DistributionGroup $path -DomainController $dc -ErrorAction SilentlyContinue
      }
      else {
        $identity = Get-DistributionGroup $path -ErrorAction SilentlyContinue
      }
    }
    else {
      # For other objects, this is the located item
      $identity = $res;
    }
    if ($identity -ne $null) {
