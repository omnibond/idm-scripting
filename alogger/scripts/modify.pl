...
$i->logger($global_config->{TRACEPRIO}, "modify.pl", "***".$ENV{'BASENAME'}."/modify.pl *** ");
...
if ($result == 1) {
  ...
  $i->status_success("Modified DirXMLjnsuNetworkDevice jnsu-$hw_addr");
}
else{
  $i->status_error("Error addding DirXMLjnsuNetworkDevice $hw_addr ".$DBI::errstr);
}
